...
from utils import get_web_element_rect, encode_image, extract_information, print_message,\
    get_webarena_traversal_tree, get_pdf_retrieval_ans_from_assistant, clip_message_and_obs, clip_message_and_obs_text_only
...
def driver_config(args):
    ...
    options.add_experimental_option(
        "prefs", {
            "download.default_directory": args.download_dir, # set download directory
...
def call_llama_api(args, model_client, messages):
    retry_times = 0
    while True:
        try:
            if not args.text_only:
                logging.info('Calling model API...')
                llama_response = model_client.chat.completions.create(
            ...
            prompt_tokens = llama_response.usage.prompt_tokens
            completion_tokens = llama_response.usage.completion_tokens
            logging.info(f'Prompt Tokens: {prompt_tokens}; Completion Tokens: {completion_tokens}')

            model_call_error = False
            return prompt_tokens, completion_tokens, model_call_error, llama_response
            ...
            if type(e).__name__ == 'RateLimitError':
                ...
                return None, None, model_call_error, None
            else:
                model_call_error = True
                return None, None, model_call_error, None
        retry_times += 1
...
def call_model_api(args, openai_client, messages):
    retry_times = 0
    while True:
        try:
            if not args.text_only:
            ...
            model_call_error = False
            return prompt_tokens, completion_tokens, model_call_error, openai_response
...
def main():
    ...
    parser.add_argument('--test_file', type=str, default='data/task_test.json')
    parser.add_argument('--max_iter', type=int, default=5)
    parser.add_argument("--api_key", default="key", type=str, help="YOUR_OPENAI_API_KEY")
    parser.add_argument("--api_model", default="llama3.2", type=str, help="api model name")
    ...
    for task_id in range(len(tasks)):
        ...
        while it < args.max_iter:
            ...
            if not fail_obs:
                try:
                    if not args.text_only:
                        ...
                        ac_tree, obs_info = get_webarena_traversal_tree(driver_task, accessibility_tree_path)
                except Exception as e:
                    if not args.text_only:
                ...
                if (not args.text_only) and args.save_accessibility_tree:
                    accessibility_tree_path = os.path.join(task_dir, 'accessibility_tree{}'.format(it))
                    get_webarena_traversal_tree(driver_task, accessibility_tree_path) 
            ...
            # call model
            # prompt_tokens, completion_tokens, model_call_error, openai_response = call_model_api(args, client, messages)
            prompt_tokens, completion_tokens, model_call_error, openai_response = call_llama_api(args, client, messages)
            ...
            # extract action info
            try:
                assert 'Thought:' in model_res and 'Action:' in model_res
