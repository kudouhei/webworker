...
class TraversalTreeNode(TypedDict):
    nodeId: str
    ignored: bool
    role: dict[str, Any]
...
TraversalTree = list[TraversalTreeNode]
...
def fetch_browser_info(browser) -> BrowserInfo:
    # handle DOM tree
    # Devtools协议是一种用于与浏览器开发工具通信的协议，它允许开发者通过客户端与浏览器进行交互，获取页面的各种信息
    # Selenium的webDriver类中的 execute_cdp_cmd(self, cmd, cmd_args) 方法可用来执行Chrome开发这个工具命令
    ...
    # Selenium 的 execute_script 方法允许在浏览器上下文中执行JavaScript 代码
...
# backendNodeId是Devtools协议中用于唯一标识元素的属性, 根据backendNodeId获取元素
def get_bounding_client_rect(
    ...
    try:
        ...
        # use Runtime 与JavaScript的执行环境进行交互, 通过Runtime域，可以在页面上下文中执行js
...
def fetch_page_traversal_tree(
    info: BrowserInfo,
    browser,
    current_viewport_only: bool,
) -> TraversalTree:
    traversal_tree: TraversalTree = browser.execute_cdp_cmd(
        "Accessibility.getFullAXTree", {}
    )["nodes"]

    # some nodes repeated in the tree
    seen_ids = set()
    _traversal_tree = []
    for node in traversal_tree:
        if node["nodeId"] not in seen_ids:
            _traversal_tree.append(node)
            seen_ids.add(node["nodeId"])
    traversal_tree = _traversal_tree

    nodeid_to_cursor = {}
    for cursor, node in enumerate(traversal_tree):
    ...
    if current_viewport_only:
        ...
        def remove_node_in_graph(node: TraversalTreeNode) -> None:
            ...
            assert(traversal_tree[parent_cursor].get("parentId", "Root") is not None)

            # remove the nodeid from parent's childIds
            index = traversal_tree[parent_cursor]["childIds"].index(nodeid)
            traversal_tree[parent_cursor]["childIds"].pop(index)

            # Insert children_nodeids in the same location
            for child_nodeid in children_nodeids:
                traversal_tree[parent_cursor]["childIds"].insert(index, child_nodeid)
                index += 1

            # update children node's parent
            for child_nodeid in children_nodeids:
                child_cursor = nodeid_to_cursor[child_nodeid]
                traversal_tree[child_cursor]["parentId"] = parent_nodeid
            
            # mark as removed
            traversal_tree[node_cursor]["parentId"] = "[REMOVED]"
...
def parse_traversal_tree(
    traversal_tree: TraversalTree,
) -> tuple[str, dict[str, Any]]:
    """Parse the tree into a string text"""
    node_id_to_idx = {}
    for idx, node in enumerate(traversal_tree):
    ...
    tree_str = dfs(0, traversal_tree[0]["nodeId"], 0)
    return tree_str, obs_nodes_info


def clean_traversal_tree(tree_str: str) -> str:
    """further clean tree"""
